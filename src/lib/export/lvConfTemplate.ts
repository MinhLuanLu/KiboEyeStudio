// Verbatim copy of LVGL 9.5.0's own `lv_conf_template.h` (sourced directly from a real installed
// LVGL 9.5.0 library, not memory/docs — same ground-truth-from-disk discipline this whole LVGL v9
// export port was built against). Two lines are pre-patched here (LV_BUILD_EXAMPLES/LV_BUILD_DEMOS
// -> 0, this export never needs LVGL's own example/demo code) — everything else is byte-for-byte
// the real template. Used by generateLvConf() in lvglExport.ts, which flips the "Content enable"
// guard on and turns on the handful of Montserrat font sizes this exporter's generated styles
// reference — everything else is left at LVGL's own stock defaults, deliberately, so this file
// behaves like the real template a user would otherwise hand-copy, rather than risking a
// hand-authored subset producing a different, more confusing error.
export const LV_CONF_TEMPLATE_V9 = `/**
 * @file lv_conf.h
