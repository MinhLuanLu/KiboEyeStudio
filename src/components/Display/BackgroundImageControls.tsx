      ...
      {!bg ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <select
              className="studio-input text-xs bg-studio-panel2 text-studio-text"
              value={bg.fitMode}
              onChange={(e) => { checkpoint(); updateBackgroundImage({ fitMode: e.target.value as BackgroundFitMode }) }}
            >
              {FIT_MODES.map((m) => (
                // Native dropdown lists render on an OS-white background, so the dark theme's light
                // text becomes invisible (light-on-white). Force a readable dark-bg/light-text pair
                // per option so the open list is legible on every platform.
                <option key={m.value} value={m.value} style={{ backgroundColor: '#212227', color: '#e6e6ea' }}>{m.label}</option>
              ))}
            </select>
