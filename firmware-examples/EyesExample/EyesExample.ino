 ...
 * EyesExample.ino
 *
 * Minimal ESP32 + GC9A01 example that plays an Eyes Eye Studio animation, using
 * EyesBufferedDisplay for flicker-free rendering (draws into an offscreen buffer, then
 * blits it in one SPI burst — see the comment above EyesBufferedDisplay in
 * EyesPlayer.h for why plain Adafruit_GC9A01A flickers without this).
 *
 * SETUP:
 *   1. Export a C++ Header from Eyes Eye Studio and save it into THIS folder as "eyes.h"
 ...
 *
 * Using TFT_eSPI or LovyanGFX instead? Skip EyesBufferedDisplay and pass your own
 * sprite/canvas object as the template type to eyesDrawEyePair() — those libraries have
 * their own buffered-sprite equivalents.
 */
#include "eyes.h"          // <- your exported header, renamed to eyes.h in this folder
#include "EyesPlayer.h"    // <- the player/draw helpers + EyesBufferedDisplay
...
void loop() {
  LiveEye live;
  bool stillPlaying = eyesPlayAnimation(activeFrames, activeCount, activeLoop, animStart, frameIndex, live);

  tft.fillScreen(BG_COLOR);
  eyesDrawEyePair(tft, 120, 120, live, BG_COLOR);  // 120,120 = center of a 240x240 panel
 ...
 *
 * NOTE on RAM: EyesBufferedDisplay allocates one GFXcanvas16 sized to your panel
